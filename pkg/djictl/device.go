...
type Device struct {
	...
	ReceivedMessageChan                    map[MessageType]chan *Message
}
...
func NewDevice(
	...
	return &Device{
		...
		ReceivedMessageChan:                    make(map[MessageType]chan *Message),
	}
}
...
func (d *Device) Init(ctx context.Context) (_err error) {
	...
	case <-ctx.Done():
		return ctx.Err()
	case msg := <-d.getReceiveMessageChan(ctx, MessageTypeMaybeStatus):
...
func (d *Device) receiveNotification(
	...
	if err != nil {
		logger.Errorf(ctx, "unable to parse the Message: %v", err)
		return
	}
	logger.Debugf(ctx, "received Message: %#+v", msg)
...
func (d *Device) getReceiveMessageChan(
	...
	return xsync.DoR1(ctx, &d.ReceiveLocker, func() chan *Message {
		if d.ReceivedMessageChan[msgType] == nil {
			d.ReceivedMessageChan[msgType] = make(chan *Message, 1)
		}
